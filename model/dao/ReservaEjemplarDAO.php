...
final class ReservaEjemplarDAO extends DAO implements InterfaceDAO{
...
public function traerReservas($idRes){
    $sql= "SELECT ejemplar from ejemplarreserva WHERE reserva= :id";
    $stm = $this->conn->prepare($sql);
    $stm->execute(array(
        "id" => $idRes
       
    ));
    return array_column($stm->fetchAll(\PDO::FETCH_ASSOC),'ejemplar');
...
public function save($ej){
  ...
  //echo"ola soy daosave el socio id es",$socio->getIdSocio();
    ...
    $sql = "INSERT INTO ejemplarreserva VALUES(DEFAULT,:idEjemplar , :idReserva)";
    $stm = $this->conn->prepare($sql);
    $stm->execute(array(
        "idReserva" => $ej->getIdReserva(),
 ...
 public function socioCantPres($socio): int{
    $sql="SELECT COUNT(*) AS cantTot from (SELECT id from prestamos where socio=:socio AND estado=1 UNION ALL SELECT id from reservas where socio=:socio AND estado= 1) AS SUMA";
    $stm=$this->conn->prepare($sql);
    $stm->execute(array(
        "socio"=> $socio));
...
public function buscarEjemXLibroISBN($titulo): bool{
    ...
    $sql= "SELECT ep.* ROM ejemplarreserva ep
...
public function socioTieneEjem($socio,$ejem){
    $sql="SELECT * from ejemplarreserva JOIN prestamos ON ejemplarprestamo.prestamo=prestamos.id WHERE prestamos.socio= '$socio' AND ejemplarprestamo.ejemplar='$ejem' AND ejemplarprestamo.fechadev='' ";
    $stmt = $this->conn->prepare($sql);
    if(!$stmt->execute()){
        throw new \Exception("No se pudo ejecutar la consulta de LISTAR");
    }
...
public function buscarEjemsRes($id){  
    ...
    $sql = "SELECT 
            ...
            FROM ejemplarreserva
            INNER JOIN ejemplares ON ejemplarreserva.ejemplar = ejemplares.codigo
            INNER JOIN libro ON ejemplares.libro = libro.id
            WHERE ejemplarreserva.reserva = :id";
          $stmt = $this->conn->prepare($sql);
          $stmt->execute(array(
            "id"=> $id));
          return $stmt->fetchAll(\PDO::FETCH_ASSOC);
...
public function renovar($pres){
    $sql= "UPDATE ejemplarreserva SET cantRenovaciones= 1 WHERE prestamo= :pres";
    $stmt = $this->conn->prepare($sql);
   $stmt->execute(array(
    "pres"=> $pres));
    return $stmt->fetchAll(\PDO::FETCH_ASSOC);
