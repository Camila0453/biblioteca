...
final class BajaLibroDAO extends DAO implements InterfaceDAO{
    public function __construct($conn){
        parent::__construct($conn);
    }
    public function load($id): BajaLibroDAO{
    
    $baja= new BajaLibroDAO($id,);
    ...
    public function save($baja):void{
        ...
        $sql = "INSERT INTO bajaslibro VALUES( DEFAULT,:libro,:usuario,NOW(),:motivo)";
        $stm = $this->conn->prepare($sql);
        $stm->execute(array(
            "motivo" => $baja->getMotivo(),
            "usuario"=> $baja->getIdUsuario(),
            "libro" => $baja->getIdLibro(),
